...
- Последний коммит: `3da4382 feat: move object photo maps to a single atlas page`, отправлен в `origin/main` (`12b452d..3da4382`).
- Workflows по этому коммиту: «SAO photo service quality» (run 3), «SMM overlay & atlas checks» (run 27), «pages build and deployment» (run 101) — все `success`.
...
12. **Выкатано на production**: коммит `3da4382` в `origin/main`, страница опубликована на GitHub Pages; в `/opt/sao-photo-service` доставлена версия, применены миграции 002–003, пересобран образ, повторён импорт, в `.env` добавлено `PHOTO_SERVICE_COOKIE_SAMESITE=none`, созданы 16 районных учёток.
...
- Живая проверка опубликованной страницы против production API (`odh-map/tests/photo-atlas-live.mjs`, только чтение) — 13 из 13, включая кросс-доменный вход.
- Перед выкатом создана и проверена резервная копия `/var/backups/sao-photo-service/20260915T065243Z` (`sha256sum -c` успешен).
...
- **Реальных фиксаций ещё нет.** Создано 16 учёток, но ни одного фото. Первую загрузку нужно проследить целиком: оригинал, миниатюра, попадание миниатюры в Excel.
...
- Резервная копия перед выкатом: `/var/backups/sao-photo-service/20260915T065243Z`. Откат — вернуть `code-before.tar.gz` и предыдущий образ.
