...
- `GET /reports/export.xlsx`, `/reports/export-headquarters.xlsx`, `/reports/export-headquarters.pdf` и `/reports/export.pdf` — **только `prefecture_admin`**, иначе `403 prefecture_role_required`. Районная учётка выгрузок не имеет вообще: её единственное действие — `POST /photos`.
