...
- Раскладка листа — по эталону заказчика: «№» и «Район», три категории и «Итого», в каждой категории «План», «Факт», «%». Ниже та же таблица, отсортированная по «Итого, %» формулой `SORT`, и готовый текст комментария к рассылке. Комментарий начинается фиксированной строкой «Направление — «Оцифровка объектов САО» — дата и время», дальше пустая строка и разбор именно этой выгрузки: общий процент и число отметок, районы без единой закрытой отметки, лидеры и самая слабая категория. Те же строки уходят подписью к картинке в Telegram.
