# Треки и проектные линии СММ (необязательный каталог)

Сюда кладутся **реальные** треки эталонных дворов после натурных заездов.
До заезда проектную линию по приложенному чертежу можно хранить в том же
формате GPX, но в паре с метаданными `source_kind: "inner_yard_reference"`.
Такая линия не является GPS-треком и в карте помечается как требующая полевой
сверки.
...
|---|---|---|
| `dt1.gpx` … `dt5.gpx` | Реальный трек или проектная линия по чертежу (GPX 1.1, любые `<trkseg>`) | `route_direction` = LineString; источник и статус берутся из метаданных пары |
| `dt1.nozzle.json` … | Замеры или проектные направления выброса: точки с азимутами | `nozzle_direction` с полем `nozzle_track` — список `[lng, lat, bearing]` |
